...
use soroban_sdk::{
    testutils::{
                Address as _,
            }, token::TokenClient, vec, Address, BytesN, Env, IntoVal, String, Symbol, Val, Vec
};
...
impl<'a> CometAggregatorAdapterTest<'a> {
    fn setup() -> Self{
        ...
        let comet_address = comet_factory_client.new_c_pool(&BytesN::from_array(&e, &[0; 32]), &admin, &vec![&e, token_a.address.clone(), token_b.address.clone()], &vec![&e, 8000000,  2000000], &vec![&e, 10000000000000,  3200000000], &30000);
        let comet_client = CometPairClient::new(&e, &comet_address);

        let adapter_wasm_hash = e.deployer().upload_contract_wasm(comet_adapter_contract::WASM);
