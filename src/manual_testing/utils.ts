...
const deployAndMint = async (asset: Asset, user: Keypair, amount: string) => {
  ...
  await mintToken(
    ...
    loadedConfig.horizonRpc,
    loadedConfig.passphrase
  );
...
const create_soroswap_liquidity_pool = async (router: string, poolParams: CreatePoolParams) => {
  ...
  const soroswapInvoke = await invokeCustomContract(
    router,
    'add_liquidity',
    addSoroswapLiquidityParams,
    poolParams.user
  );
...
const createSoroswapLP = async (
  ...
  try {
    const soroswapPool = await invokeCustomContract(
      router,
      'router_pair_for',
      fetchPoolParams,
      user,
    ...
    const soroswapPoolBalance = await invokeCustomContract(
      soroswapPoolCID,
      'get_reserves',
      [],
      user,
...
const create_phoenix_liquidity_pool = async (
  phoenixAdmin: Keypair,
  aggregatorAdmin: Keypair,
  assetA: Asset,
  assetB: Asset
  ...
  const factory_contract = new PhoenixFactoryContract.Client({
    ...
    signTransaction: async (tx: string) => {
      const signedTxXdr = await signWithKeypair(tx, loadedConfig.passphrase, phoenixAdmin);
      return { signedTxXdr };
    },
  });
  ...
  try {
    ...
    await create_phoenix_pool_transaction(
      factory_contract,
      phoenixAdmin,
      aggregatorAdmin,
      assetA,
...
const provide_phoenix_liquidity = async (
  phoenixAdmin: Keypair,
  pairAddress: string,
  amount_A: number,
  amount_B: number
  ...
  const provide_liquidity = await invokeCustomContract(
    pairAddress,
    'provide_liquidity',
    addPhoenixLiquidityParams,
    phoenixAdmin
  );
    ...
    return xdr.ScVal.scvMap([
      ...
      new xdr.ScMapEntry({
        ...
        val: xdr.ScVal.scvVec(
          distribution.path.map((pathAddress) => new Address(pathAddress).toScVal())
        ),
      }),
      new xdr.ScMapEntry({
...
const callAggregatorSwap = async (
  asset_a: string,
  asset_b: string,
  max_amount: number,
  dexDistributionScValVec: xdr.ScVal,
  user: Keypair,
  method: SwapMethod
) => {
