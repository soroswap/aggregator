...
export async function cometSetup(loadedConfig: EnvConfig, addressBook: AddressBook) {
  ...
  await invokeContract(
    ...
    [
      ...
      nativeToScVal([new Address(tokenA.contract).toScVal(), new Address(tokenB.contract)]),
      nativeToScVal([
      ...
      nativeToScVal([
        ...
        nativeToScVal(250000000, { type: 'i128' }),
      ]),
  ...
  const initArgs = xdr.ScVal.scvVec([
    ...
    new Address('CBLV7M5FJL7QAUZLNTGAALBDN4BFZ6CFTWYUMSKAGIEMVPSZU7JZGVRP').toScVal(), // protocol_address as ScVal address
  ]);
